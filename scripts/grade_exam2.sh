...
if id -u op1 &>/dev/null && [ "$(id -u op1)" == "2500" ] && [ "$(id -g op1)" == "$(getent group sysadmins | cut -d: -f3)" ] && [ "$(getent group sysadmins | cut -d: -f3)" == "3500" ] && [ "$(getent group operators | cut -d: -f3)" == "3501" ] && id op1 | grep -q operators && id -u op2 &>/dev/null && [ "$(id -u op2)" == "2501" ] && [ "$(id -g op2)" == "$(getent group operators | cut -d: -f3)" ] && passwd -S op2 2>/dev/null | awk '{print $2}' | grep -q LK; then
...
if [ -f /etc/sudoers.d/op1 ] && [ "$(stat -c %a /etc/sudoers.d/op1)" == "440" ] && grep -q "op1" /etc/sudoers.d/op1 && grep -q "ALL=(ALL)" /etc/sudoers.d/op1 && grep -q "NOPASSWD" /etc/sudoers.d/op1; then
    SUDO_OK=1
fi
...
if [ -d /srv/exam2 ] && [ "$(stat -c %a /srv/exam2)" == "2770" ] && [ -f /usr/local/bin/exam2_cmd ] && [ "$(stat -c %a /usr/local/bin/exam2_cmd)" == "4750" ]; then
...
# Check 5: ACLs (5 pts)
if [ -d /audit/exam2 ] && getfacl /audit/exam2 2>/dev/null | grep -q "^user:op1:" && getfacl /audit/exam2 2>/dev/null | grep -q "^default:user:op1:"; then
...
if pvs 2>/dev/null | grep -q /dev/sdb1 && vgs 2>/dev/null | grep -q vg_exam2 && lvs 2>/dev/null | grep -q "lv_app" && findmnt -n /mnt/app &>/dev/null && swapon -s | grep -q lv_swap; then
...
if awk -v free="$VG_FREE" 'BEGIN { exit !(free < 200) }' 2>/dev/null && grep -q nfsserver.example.com:/exports/exam2 /etc/fstab; then
...
if systemctl is-active autofs &>/dev/null && grep -q "/remote/users" /etc/auto.master 2>/dev/null; then
...
if [[ "$MODE" == "enforcing" && -d /app ]] && ls -Zd /app 2>/dev/null | awk '{print $4}' | grep -q "httpd_sys_content_t" && getsebool httpd_can_network_connect 2>/dev/null | grep -q "on" && getsebool httpd_can_network_relay 2>/dev/null | grep -q "on" && semanage port -l 2>/dev/null | grep "http_port_t" | grep -q "8443"; then
...
if systemctl is-active firewalld &>/dev/null && firewall-cmd --list-services --permanent | grep -qE "http|https|ssh|smtp" && firewall-cmd --list-ports --permanent | grep -q "8443/tcp" && firewall-cmd --list-ports --permanent | grep -q "8081/tcp"; then
...
if crontab -l 2>/dev/null | grep -q "archive_logs.sh" && atq 2>/dev/null | grep -q "notify.sh" && [ -x /usr/local/bin/archive_logs.sh ] && systemctl is-enabled exam2.timer &>/dev/null; then
...
if podman ps --format "{{.Names}}" 2>/dev/null | grep -q "^exam2-web$" && systemctl is-enabled container-exam2-web &>/dev/null; then
...
if [ -x /usr/local/bin/check_users.sh ] && /usr/local/bin/check_users.sh op1 2>/dev/null | grep -q "EXIST" && /usr/local/bin/check_users.sh missinguser 2>/dev/null | grep -q "MISSING"; then

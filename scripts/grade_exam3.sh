...
if id -u user1 &>/dev/null && [ "$(id -u user1)" == "2600" ] && [ "$(id -g user1)" == "$(getent group staff | cut -d: -f3)" ] && [ "$(getent group staff | cut -d: -f3)" == "3600" ] && [ "$(getent group students | cut -d: -f3)" == "3601" ] && id user1 | grep -q students && id -u user2 &>/dev/null && [ "$(id -u user2)" == "2601" ] && [ "$(id -g user2)" == "$(getent group students | cut -d: -f3)" ] && passwd -S user2 2>/dev/null | awk '{print $2}' | grep -q LK; then
...
HOME_OK=0
AGING_OK=0
if [ -d /home/user1 ] && [ "$(stat -c %a /home/user1)" == "700" ] && [ "$(stat -c %U:%G /home/user1)" == "user1:user1" ]; then
    HOME_OK=1
fi
...
if [ -d /project/exam3 ] && [ "$(stat -c %a /project/exam3)" == "2770" ] && [ "$(stat -c %U:%G /project/exam3)" == "root:students" ] && grep -q "^UMASK.*0077" /etc/login.defs; then
...
# Check 5: ACLs (5 pts)
if [ -d /restricted/exam3 ] && getfacl /restricted/exam3 2>/dev/null | grep -q "^user:user1:" && getfacl /restricted/exam3 2>/dev/null | grep -q "^default:user:user1:"; then
...
if pvs 2>/dev/null | grep -q /dev/sdb1 && vgs 2>/dev/null | grep -q vg_exam3 && lvs 2>/dev/null | grep -q "lv_docs" && findmnt -n /mnt/docs &>/dev/null && swapon -s | grep -q lv_swap; then
...
if awk -v free="$VG_FREE" 'BEGIN { exit !(free < 200) }' 2>/dev/null && grep -q "LABEL=EXAM3_DOCS" /etc/fstab; then
...
if grep -q nfsserver.example.com:/exports/exam3 /etc/fstab && grep -q "/remote/docs" /etc/auto.master 2>/dev/null && systemctl is-active autofs &>/dev/null; then
...
if [[ "$MODE" == "enforcing" && -d /webdocs ]] && ls -Zd /webdocs 2>/dev/null | awk '{print $4}' | grep -q "httpd_sys_content_t" && getsebool httpd_can_sendmail 2>/dev/null | grep -q "on" && getsebool httpd_use_nfs 2>/dev/null | grep -q "on" && semanage port -l 2>/dev/null | grep "http_port_t" | grep -q "8888"; then
...
if systemctl is-active firewalld &>/dev/null && firewall-cmd --list-services --permanent | grep -qE "http|https|ssh|nfs" && firewall-cmd --list-ports --permanent | grep -q "8888/tcp"; then
...
if crontab -l 2>/dev/null | grep -q "cleanup_tmp.sh" && atq 2>/dev/null | grep -q "send_report.sh" && [ -x /usr/local/bin/cleanup_tmp.sh ]; then
...
if [ -x /usr/local/bin/list_large_files.sh ] && /usr/local/bin/list_large_files.sh /etc &>/dev/null; then
...
if [ -f /etc/yum.repos.d/exam3.repo ] && rpm -q httpd bash-completion nfs-utils &>/dev/null && [ -f /etc/logrotate.d/exam3 ]; then
...
if [ -f /tmp/exam3_denied.txt ] && [ -f /etc/rsyslog.d/exam3_local0.conf ]; then

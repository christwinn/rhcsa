...
if id -u admin1 &>/dev/null && [ "$(id -u admin1)" == "3000" ] && [ "$(id -g admin1)" == "$(getent group admins | cut -d: -f3)" ] && [ "$(getent group admins | cut -d: -f3)" == "4500" ] && [ "$(getent group users | cut -d: -f3)" == "4501" ] && id admin1 | grep -q users && id -u user01 &>/dev/null && [ "$(id -u user01)" == "3001" ] && [ "$(id -g user01)" == "$(getent group users | cut -d: -f3)" ] && passwd -S user01 2>/dev/null | awk '{print $2}' | grep -q LK; then
...
if [ -f /etc/sudoers.d/admin1 ] && [ "$(stat -c %a /etc/sudoers.d/admin1)" == "440" ] && grep -q "admin1" /etc/sudoers.d/admin1 && grep -q "ALL=(ALL)" /etc/sudoers.d/admin1 && grep -q "NOPASSWD" /etc/sudoers.d/admin1; then
    SUDO_OK=1
fi
...
if [ -d /collab/exam4 ] && [ "$(stat -c %a /collab/exam4)" == "2770" ] && [ -d /collab/tmp ] && [ "$(stat -c %a /collab/tmp)" == "1777" ] && [ -f /usr/local/bin/exam4_exec ] && [ "$(stat -c %a /usr/local/bin/exam4_exec)" == "4750" ]; then
...
# Check 5: ACLs (5 pts)
if [ -d /confidential/exam4 ] && getfacl /confidential/exam4 2>/dev/null | grep -q "^user:admin1:" && getfacl /confidential/exam4 2>/dev/null | grep -q "^default:user:admin1:"; then
...
if pvs 2>/dev/null | grep -q /dev/sdb1 && vgs 2>/dev/null | grep -q vg_exam4 && lvs 2>/dev/null | grep -q "lv_data" && findmnt -n /mnt/data &>/dev/null && swapon -s | grep -q lv_swap; then
...
if awk -v free="$VG_FREE" 'BEGIN { exit !(free < 200) }' 2>/dev/null && grep -q "LABEL=EXAM4_DATA" /etc/fstab; then
...
if grep -q nfsserver.example.com:/exports/exam4 /etc/fstab && grep -q "/remote/exam4" /etc/auto.master 2>/dev/null && systemctl is-active autofs &>/dev/null; then
...
if [[ "$MODE" == "enforcing" && -d /webapp ]] && ls -Zd /webapp 2>/dev/null | awk '{print $4}' | grep -q "httpd_sys_content_t" && getsebool httpd_can_network_connect 2>/dev/null | grep -q "on" && getsebool httpd_can_sendmail 2>/dev/null | grep -q "on" && semanage port -l 2>/dev/null | grep "http_port_t" | grep -q "8090"; then
...
if systemctl is-active firewalld &>/dev/null && firewall-cmd --list-services --permanent | grep -qE "http|https|ssh|smtp" && firewall-cmd --list-ports --permanent | grep -q "8090/tcp"; then
...
if [[ "$TARGET" == "multi-user.target" ]] && systemctl is-active httpd sshd chronyd crond &>/dev/null; then
...
if crontab -l 2>/dev/null | grep -q "report.sh" && atq 2>/dev/null | grep -q "send_alert.sh" && [ -x /usr/local/bin/report.sh ] && systemctl is-enabled exam4.timer &>/dev/null; then
...
if [ -f /etc/yum.repos.d/exam4.repo ] && grep -qi "baseos" /etc/yum.repos.d/exam4.repo && rpm -q httpd bash-completion tmux nfs-utils &>/dev/null && systemctl is-active chronyd &>/dev/null; then
...
if [ -x /usr/local/bin/disk_usage.sh ] && /usr/local/bin/disk_usage.sh / &>/dev/null; then
...
if [ -f /etc/logrotate.d/exam4 ] && grubby --info=DEFAULT 2>/dev/null | grep -q "audit=1" && grubby --info=DEFAULT 2>/dev/null | grep -q "quiet"; then
...
if grep -q "pool.ntp.org" /etc/chrony.conf 2>/dev/null && [ -f /etc/rsyslog.d/exam4.conf ]; then
...
if [ -f /swapfile ] && swapon -s | grep -q /swapfile && tuned-adm active 2>/dev/null | grep -q "balanced"; then

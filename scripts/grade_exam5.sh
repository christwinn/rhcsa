...
if id -u examuser1 &>/dev/null && [ "$(id -u examuser1)" == "1900" ] && [ "$(id -g examuser1)" == "$(getent group examusers | cut -d: -f3)" ] && [ "$(getent group examusers | cut -d: -f3)" == "3000" ] && id examuser1 | grep -q examusers && id -u examuser2 &>/dev/null && [ "$(id -u examuser2)" == "1901" ] && passwd -S examuser2 2>/dev/null | awk '{print $2}' | grep -q LK && [ -f /etc/sudoers.d/examuser1 ] && [ "$(stat -c %a /etc/sudoers.d/examuser1)" == "440" ] && grep -q "examuser1" /etc/sudoers.d/examuser1 && grep -q "ALL=(ALL)" /etc/sudoers.d/examuser1 && grep -q "NOPASSWD" /etc/sudoers.d/examuser1; then
...
HOME_OK=0
AGING_OK=0
if [ -d /home/examuser1 ] && [ "$(stat -c %a /home/examuser1)" == "700" ] && [ "$(stat -c %U:%G /home/examuser1)" == "examuser1:examuser1" ]; then
    HOME_OK=1
fi
...
if [ -d /data/exam ] && [ "$(stat -c %a /data/exam)" == "2770" ] && [ "$(stat -c %U:%G /data/exam)" == "root:examusers" ]; then
...
# Check 5: ACLs (5 pts)
if [ -d /secure/exam ] && getfacl /secure/exam 2>/dev/null | grep -q "^user:examuser1:" && getfacl /secure/exam 2>/dev/null | grep -q "^default:user:examuser1:"; then
...
# Check 6: LVM (6 pts)
if pvs 2>/dev/null | grep -q /dev/sdb1 && vgs 2>/dev/null | grep -q vg_final && lvs 2>/dev/null | grep -q "lv_final" && findmnt -n /mnt/final &>/dev/null; then
...
if awk -v free="$VG_FREE" 'BEGIN { exit !(free < 200) }' 2>/dev/null && [ -b /dev/vg_final/lv_swap ] && swapon -s | grep -q lv_swap; then
...
if grep -q nfsserver.example.com:/exports/exam5 /etc/fstab && findmnt -n /mnt/labeled &>/dev/null; then
...
if systemctl is-active autofs &>/dev/null && grep -q "/remote/exam" /etc/auto.master 2>/dev/null; then
...
if [[ "$MODE" == "enforcing" && -d /web ]] && ls -Zd /web 2>/dev/null | awk '{print $4}' | grep -q "httpd_sys_content_t" && getsebool httpd_can_network_connect 2>/dev/null | grep -q "on" && semanage port -l 2>/dev/null | grep "http_port_t" | grep -q "8443"; then
...
if systemctl is-active httpd &>/dev/null && podman ps --format "{{.Names}}" 2>/dev/null | grep -q "^examweb$" && systemctl is-enabled container-examweb &>/dev/null; then
...
if [[ "$TARGET" == "multi-user.target" ]] && systemctl is-active httpd sshd chronyd crond firewalld &>/dev/null && systemctl is-enabled httpd sshd chronyd crond firewalld &>/dev/null; then
...
if grep -q "pool.ntp.org" /etc/chrony.conf 2>/dev/null && [ -f /etc/rsyslog.d/exam_secure.conf ] && sysctl -n kernel.randomize_va_space 2>/dev/null | grep -q "2" && [ -d /var/log/journal ]; then
...
if crontab -l 2>/dev/null | grep -q "exam_backup" && atq 2>/dev/null | grep -q "exam_alert" && systemctl is-enabled exam.timer &>/dev/null; then
...
if [ -f /etc/yum.repos.d/exam5.repo ] && grep -qi "baseos" /etc/yum.repos.d/exam5.repo && grep -qi "appstream" /etc/yum.repos.d/exam5.repo && rpm -q httpd bash-completion tmux nfs-utils &>/dev/null; then
...
if systemctl is-active tuned &>/dev/null && tuned-adm active 2>/dev/null | grep -q "virtual-guest" && [ -f /etc/logrotate.d/exam5 ]; then
